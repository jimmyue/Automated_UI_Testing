...
class WeChat:
    def __init__(self):
        #企业ID，在管理后台获取
        self.CORPID = os.getenv("WX_CORPID")
        #自建应用的Secret，每个自建应用里都有单独的secret
        self.CORPSECRET = os.getenv("WX_CORPSECRET")
        #应用ID，在后台应用中获取
        self.AGENTID = os.getenv("WX_AGENTID")
        #接收者用户名,多个用户用|分割
        self.TOUSER = "@all"  
        #接收部门ID，多个部门用|分割
        self.TOPARTY=os.getenv("WX_TOPARTY")
...
if __name__ == '__main__':
    wx = WeChat()
    mediaid=wx.upload_media('猫猫.png')
    wx.send_image(mediaid)
